...
function parsePayslip(str): any {
	const res: any = {};
	const dates = /(Benjamin Herila).*?([/\d]{10})\s+([/\d]{10})\s*([/\d]{10})/.exec(str);
	if (dates) {
	...
	const r2 = str.split('\n').map(line => /(Performance Bonus|SEVER|Holiday|Salary|(?:Non-Taxable )?Stipend)[^.]+\.\d{2,6}\s+([\d,]+\.\d{2,6})\s+([\d,]+\.\d{2})/i.exec(line)).filter(x => !!x);
	for (const match of r2) {
...
export function NewPayslip(props: {onSubmit?: (any) => void}) {
	...
	return <form onSubmit={(e) => {
		...
		<textarea
			style={{width: '100%'}}
			onChange={e => setPayslip(e.currentTarget.value)} rows={10} placeholder={"Paste here"} className={"textbox"}>{payslip}</textarea>
	...
	return <Container fluid>
		<Row>
			<Col sm={8}>
				...
				<DataGrid
					dataSource={payslips}
					onRowRemoving={(e) => setPayslips(payslips.filter(x => x !== e.data))}
				>
					<Editing
						mode="row"
						allowUpdating={false}
