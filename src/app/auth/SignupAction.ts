...
export async function SignupAction(formData: FormData): Promise<void> {
  try {
    ...
    const exist: { c: number }[] = await db.query('select count(*) as c from users where email = ?', [signup.email])
    if (exist[0].c !== 0) {
      // try login
      try {
    ...
    const { err, result, fields } = (await db.query('insert into users (email, pw, salt, alias) values (?, ?, ?, ?)', [
      signup.email,
      signup.password,
