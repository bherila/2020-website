...
import { updateAccountClosed } from './actions/update-account-closed'

interface Props {
  accountId: number
  accountName: string
  whenClosed: Date | null
}
...
export default function MaintenanceClient({ accountId, accountName, whenClosed: initialWhenClosed }: Props) {
  ...
  const [whenClosed, setWhenClosed] = useState<Date | null>(initialWhenClosed)
  ...
  const handleSaveClosedStatus = async () => {
    try {
      await updateAccountClosed(accountId.toString(), isClosed ? whenClosed : null)
      window.location.reload()
    } catch (err) {
