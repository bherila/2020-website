...
const schema: ISchema = {
  ...
  order_v4: {
    // Created when the order is placed.
    properties: {
      orderGuid: required('uuid'),
  ...
  orderDerivedData: {
    // Calculated when requested, not stored in database directly.
    properties: {
  ...
  orderLineItem: {
    // Created when the order is placed. For an order from daily offer, there would be 1 line item to order X bottles from the offer.
    properties: {
      orderGuid: required('uuid'),
...
function buildSql(name: string, model: IEntity) {
  ...
  function emitSql() {
    ...
    for (const propName of propNames) {
      colNames[propName] =
        (model.sql.columnOverride || {})[propName] || propName
    }
    const idProp = model.sql.identityProperty
    const idCol = colNames[idProp]
    if (typeof idCol === 'undefined') {
      throw `idCol ${
        model.sql.identityProperty || 'undefined'
      } not found on model ${name}`
    }
    ...
    return `
        ${
          !model.sql.readOnly
            ? `
        public const string PgSqlInsert = @"INSERT INTO ${
          model.sql.table || name
        } (${propNames.map((pn) => colNames[pn]).join(', ')}) 
            VALUES (${propNames.map((pn) => `@${camel(pn)}`).join(', ')})";

        // ReSharper disable once IdentifierTypo
        public const string PgSqlUpsert = @"INSERT INTO ${
          model.sql.table || name
        } (${propNames.map((pn) => colNames[pn]).join(', ')}) 
            VALUES (${propNames.map((pn) => `@${camel(pn)}`).join(', ')})
        ...
        public const string PgSqlSelectAll = @"SELECT ${propNames
          .map((pn) => colNames[pn])
          .join(', ')}
            FROM ${model.sql.table || name} LIMIT @take OFFSET @skip";

        public const string PgSqlSelectByIdentity = @"SELECT ${propNames
          .map((pn) => colNames[pn])
          .join(', ')}
      ...
      idProp
        ...
        public static ${cn} BuildFromDynamicRow(dynamic row) {
            return new ${cn}.Builder()${propNames
      .map(
        (pn) => `
...
function buildTableHelper(name: string, model: IEntity) {
  ...
  function emitNoSql() {
    ...
    return `
        ...
        ${['InsertOrReplace', 'InsertOrMerge', 'Insert', 'Merge', 'Replace']
          .map(
            (op) => `
...
function buildEntity<TModel extends IEntity>(name: string, model: TModel) {
  ...
  function buildValidationLogic(propName: string, prop: Argument) {
    ...
    const csType =
      gCsType(model, prop) || (prop.type in schema && camel(prop.type))
